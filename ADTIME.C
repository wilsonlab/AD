...
int        totspikes=0;
    ...
    if(sysinfo.detect_spikes){
        ...
        */
                    ...
                    if(*(dptr)-offset > *(tptr)) { 
						if (*subptr) {
							...
							*/
							...
							spikeptr->type2 = 's';
							spikeptr->electrode = e_num;
							spikeptr->timestamp = compute_timestamp(
														adinfo.process_buf, i);
							/* move the data pointer back to the beginning of
							   the buffer */
							dptr = dataptr + bufoffset;
							/* 
		...
		if(totspikes > adinfo.peak_rate) adinfo.peak_rate = totspikes;
    }
    EnableADIRQ;
}
...
//    adinfo.overlap_window_size = (SPIKELEN - PRESPIKE) * adinfo.nelectrodes;
    ...
    adinfo.dma_bufsize = adinfo.nchannels*nsamples;  /* number of data points per buffer */
    ...
    ** overlap between two data buffers */
    ...
    if(sysinfo.debug){
        gprintf(&sysinfo.debugwinx, &sysinfo.debugwiny,"Base address 0x%x\n",BASE);
    }

    if(!SetRate(adinfo.rate_req,&adinfo.rate_set)) 
		SystemExit(2,"Set Clock Error");
    /*
    ** compute the duration in units of 100 microseconds of an a/d conversion of a
    ** single buffer
    */
...
DMADONE:
            ...
            if((adcsr & BIT15) == BIT15){
                if(sysinfo.debug){
                    gprintf(&sysinfo.debugwinx, &sysinfo.debugwiny,"ERROR detected ADCSR: %X SUPCSR:%X",
                    adcsr,supcsr);
                }
                sprintf(tmpstring,"ERROR detected ADCSR: %X SUPCSR:%X",adcsr,supcsr);
#endif			
                ErrorMessage(tmpstring);
				EventString("ERROR");
                adinfo.error_count++;
...
// wait for the multiplexor to get back in sync following the error
                // skip a buffer
                StopAcq();
                // skip a buffer
                StartAcq();
