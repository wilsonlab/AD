...
{
    drwline(SET,color, x+1,y, x + 5, y);
    drwline(SET,color, x+5,y, x + 3, y-2);
    drwline(SET,color, x+3,y-2, x + 3, y+2);
    drwline(SET,color, x+3,y+2, x + 5, y);
}
...
void SetStringPositions(void)
{
	...
	sysinfo.currenttimeview.x = 0;
	sysinfo.currenttimeview.y = 0;
	sysinfo.currenttimeview.x2 = 200;
	sysinfo.currenttimeview.y2 = 2*sysinfo.theight;
	/* remaining time location */
	sysinfo.remaintimeview.x = 112;
	sysinfo.remaintimeview.y = 2*sysinfo.theight+4;
	sysinfo.remaintimeview.x2 = 180;
	sysinfo.remaintimeview.y2 = 3*sysinfo.theight + 4;
	/* file size location */
	sysinfo.filesizeview.x = 405;
	sysinfo.filesizeview.y = 0;
	sysinfo.filesizeview.x2 = 497;
	sysinfo.filesizeview.y2 = 2*sysinfo.theight;
	/* disk free space location */
	sysinfo.diskfreeview.x = 305;
	sysinfo.diskfreeview.y = 2*sysinfo.theight + 4;
	sysinfo.diskfreeview.x2 = 397;
	sysinfo.diskfreeview.y2 = 3 * sysinfo.theight + 4;
	// rate display
	sysinfo.rateview.x = 503;
	sysinfo.rateview.y = 0;
	sysinfo.rateview.x2 = 799;
	sysinfo.rateview.y2 = 6 * sysinfo.theight;
	// Version string
	sysinfo.versionview.x = 412;
	sysinfo.versionview.y = 550;
	sysinfo.versionview.x2 = 800;
	sysinfo.versionview.y2 = 550 + sysinfo.theight;
    } else {
	/* current time location */
...
char tmpstring[300];
	...
	drwfillbox(SET, LIGHTGRAY, 0,455,800, 455+2*sysinfo.theight);
	setcolor(WHITE);
time(&clock);
	sprintf(tmpstring,"%s %s",ctime(&clock),sysinfo.sessionstring);
	outtextxy(0,455, tmpstring);
	drwfillbox(SET, LIGHTGRAY, 400,455-2*sysinfo.theight,800, 455);
	setcolor(WHITE);
	sprintf(tmpstring,"%s %s",FormatTS(ReadTS() - sysinfo.epochtime,0),
	sysinfo.epochstring);
	outtextxy(400,455-2*sysinfo.theight, tmpstring);
	settextstyle(DEFAULT_FONT,HORIZ_DIR,1);
	if(sysinfo.tracker && sysinfo.disk){
		drwfillbox(SET, GREEN, sysinfo.versionview.x,sysinfo.versionview.y,
		sysinfo.versionview.x2,sysinfo.versionview.y2);
	} else {
		drwfillbox(SET, BLACK, sysinfo.versionview.x,sysinfo.versionview.y,
		sysinfo.versionview.x2,sysinfo.versionview.y2);
	}
	setcolor(WHITE);
	...
	setcolor(WHITE);
	if(strlen(sysinfo.sessionstring) == 0){
	    sprintf(tmpstring,"AD Version %s (c) 1995 M.Wilson, L.Frank, MIT",
	    ADVERSION);
	    outtextxy(sysinfo.versionview.x, sysinfo.versionview.y, tmpstring);
	} else {
	    outtextxy(sysinfo.versionview.x, sysinfo.versionview.y, sysinfo.sessionstring);
	}
    }
}
...
void DisplaySetup()
{
   ...
   */
   ...
   sprintf(electrode->projection[0].info, "1,2");
   sprintf(electrode->projection[1].info, "1,3");
   sprintf(electrode->projection[2].info, "1,4");
   sprintf(electrode->projection[3].info, "2,3");
   sprintf(electrode->projection[4].info, "2,4");
   sprintf(electrode->projection[5].info, "3,4");
   ...
   */
   ...
   for (j = 0; j < adinfo.nelectrodes; j++) {
       ...
       for (i = 0; i < adinfo.nelect_chan; i++) {
           ...
           DefineButton(++sysinfo.maxbutton, buttonx, buttony, buttonx +
                        CHANNEL_BUTTON_WIDTH, buttony + 2.4 * BUTTON_HEIGHT, 
                         NULL, ButtonStatus);
   ...
   /* setup the coordinates of the tracker mode display window */
   sysinfo.trackview.x = 49;
   sysinfo.trackview.y = 49;
   sysinfo.trackview.x2 = 50 + 328 + 1;
   sysinfo.trackview.y2 = 50 + 256 + 1;

   /* set the coordinates of the help screen window */
   sysinfo.helpview.x = 5; 
   sysinfo.helpview.y = 25; 
   sysinfo.helpview.x2 = 390; 
   sysinfo.helpview.y2 = 500; 

	/* move help and stim screens to RHS in tracker mode */
	if (sysinfo.mode == TRACKER_MODE)
	  {
     sysinfo.helpview.x = sysinfo.helpview.x + sysinfo.trackview.x2; 
     sysinfo.helpview.x2 = sysinfo.helpview.x2 + sysinfo.trackview.x2; 
     }

   /* set the coordinates of the spike viewing area so that it can be easily 
      erased */
   sysinfo.spikeview.x = 0; 
   sysinfo.spikeview.y = 40; 
   sysinfo.spikeview.x2 = 394; 
   sysinfo.spikeview.y2 = 550; 
...
{
   if(sysinfo.tracker){
       DrawButton("(T)ra ON",NULL, buttonlist[1].x, buttonlist[1].y, 
                  buttonlist[1].x2, buttonlist[1].y2, 1);
   } else {
       DrawButton("(T)ra OFF", NULL, buttonlist[1].x, buttonlist[1].y, 
                  buttonlist[1].x2, buttonlist[1].y2, 0);
   }
   DrawSessionString(); 
}
...
void DrawMenu(void)
{
   ...
   UpdateFileButton();
    ...
    case CONTINUOUS_MODE:
        ...
        break;
    case TRACKER_MODE:
         DrawButton("Tracker mode", NULL, buttonlist[4].x, buttonlist[4].y, 
                buttonlist[4].x2, buttonlist[4].y2, 1);
        break;
   }
...
void DrawStrings(void)
{
	...
	if(sysinfo.tracker_enabled){
		settextstyle(DEFAULT_FONT,HORIZ_DIR,2);
		drwfillbox(SET, LIGHTGRAY, sysinfo.filesizeview.x-textwidth("AAAAAAAA.AAA: "), 
...
void DisplayCurrentTime(void)
{
	...
	if(sysinfo.tracker_enabled){
		...
		drwfillbox(SET, LIGHTGRAY, sysinfo.currenttimeview.x, 
		...
		settextstyle(DEFAULT_FONT,HORIZ_DIR,2);
		outtextxy(sysinfo.currenttimeview.x+2, sysinfo.currenttimeview.y, 
              FormatTS(sysinfo.currenttime, 1));
		settextstyle(DEFAULT_FONT,HORIZ_DIR,1);
	} else {
...
void DrawChannelButton(int channel)
{
    ...
    if (sysinfo.mode == SPIKE_MODE) {
		...
		*/
		if(gain > 0){
			winscalemax = 10.0/(gain*(1<<adinfo.channel[channel].adgain));
		} else {
			winscalemax = 0;
		}
...
//        drwline(SET, DARKGRAY, x, y, x, buttonlist[blistnum].y2 - 1);
        ...
        if (absv(winscale) < 1.0e-3)
            sprintf(tmpstring,"  %2d  %3.0fuV", channel+1, winscale * 1.0e6);
        else if (absv(winscale) < 1.0)
            sprintf(tmpstring,"  %2d  %3.0fmV", channel+1, winscale * 1.0e3);
        else
            sprintf(tmpstring,"  %2d  %3.0fV", channel+1, winscale);

        /* write out the gain string */
        sprintf(tmpstring2,"%5ld %d %4d", gain, 1 << 
                adinfo.channel[channel % NAMP_CHANNELS].adgain, 
                adinfo.channel[channel % NAMP_CHANNELS].thresh);
   ...
   structure */
{
    ...
    */
	...
	/ (float) MAXVALUE;    
    cptr->contthreshtick = cptr->contoffset - cptr->thresh*cptr->contwindowscale;
}							
...
{
     drwbox(SET, _RED, sysinfo.trackview.x, sysinfo.trackview.y,
              sysinfo.trackview.x2, sysinfo.trackview.y2);
}
...
{
     drwfillbox(SET, _BLACK, sysinfo.trackview.x-2, sysinfo.trackview.y-2,
              sysinfo.trackview.x2+2, sysinfo.trackview.y2+2);
}

void ClearSpikeAreas(void)
{
    drwfillbox(SET, _BLACK, sysinfo.spikeview.x, sysinfo.spikeview.y,
				sysinfo.spikeview.x2, sysinfo.spikeview.y2);
}

void ClearHelpWindow(void)
{
    drwfillbox(SET, _BLACK, sysinfo.helpview.x, sysinfo.helpview.y, 
    			sysinfo.helpview.x2, sysinfo.helpview.y2);
}
...
void DrawHelpWindow(void)
{
	...
	/* draw the border */
    drwbox(SET, _RED, sysinfo.helpview.x, sysinfo.helpview.y, 
           sysinfo.helpview.x2, sysinfo.helpview.y2);
 	/* draw the title */
    setcolor(WHITE);
    sprintf(tmpstring, "AD Help Window");
    /* x and y determine the coordinates of the beginning positions of the
       help strings. The tmpstring title will be centered in the window */ 
    *x = (sysinfo.helpview.x + sysinfo.helpview.x2 - textwidth(tmpstring)) / 2; 
    *y = sysinfo.helpview.y + 2 * sysinfo.theight;
    gprintf(x, y, "%s", tmpstring);

    *x = sysinfo.helpview.x + sysinfo.twidth / 2; 
    *y = *y + 2 * sysinfo.theight;
	gprintf(x, y, " A        Acquisition toggle");
	gprintf(x, y, " c        Clear currently selected window(s)");
	gprintf(x, y, " D        Disk storage toggle");
	gprintf(x, y, "^E        Equalize amplifier");
	gprintf(x, y, " F        Open or close a data file");
	gprintf(x, y, " g        Set amp gain for selected channel");
	gprintf(x, y, "^G        Set amp gain for selected electrode");
	gprintf(x, y, "^H        Cycle high filter");
	gprintf(x, y, " L        Load a configuration file");
	gprintf(x, y, "^L        Cycle low filter");
	gprintf(x, y, " M        Enter master system command mode");
	gprintf(x, y, " o        Toggle overlay in current window");
	gprintf(x, y, " O        Toggle between modes");
	gprintf(x, y, " p        Toggle stimulating pulse output");
	gprintf(x, y, " P        Set parameters for stimulation");
	gprintf(x, y, " Q        Quit");
	gprintf(x, y, " R        Set acquisition rate");
	gprintf(x, y, " S        Save a configuration file");
	gprintf(x, y, " t        Set threshold for selected channel");
	gprintf(x, y, "^T        Set threshold for selected electrode");
	gprintf(x, y, " =/-      +/- channel thresh. by 10");
	gprintf(x, y, " +/_      +/- channel thresh. by 50 (spike)");
	gprintf(x, y, " +        Increase magnification of trace (cont)");
	gprintf(x, y, " _        Decrease magnification of trace (cont)");
	gprintf(x, y, " }        Increase board gain for channel"); 
	gprintf(x, y, " {        Decrease board gain for channel"); 
	gprintf(x, y, " Up/Dn    Move trace up/dn (cont)");
	gprintf(x, y, " Home     Toggle between proj. buttons (spike)");
	gprintf(x, y, " Home     Change color of trace (cont)");
	gprintf(x, y, " End      Change color of trace (cont)");
	gprintf(x, y, " PgUp/Dn  +/- amp gain for channel by 2x");
	gprintf(x, y, "^PgUp/Dn  +/- amp gain for electrode by 2x");
	gprintf(x, y, " Alt F1-8 Enter string for event string");
	gprintf(x, y, " F1-8     Record event string in file");
	gprintf(x, y, " F9, F10  Load preset configuration file");
	gprintf(x, y, "");
	gprintf(x, y, " Tracker commands:");
	gprintf(x, y, " T        Toggle tracker");
	gprintf(x, y, " )        Increase tracker speed");
	gprintf(x, y, " (        Decrease tracker speed");
	*y = sysinfo.helpview.y2 - 2 * sysinfo.theight;
	gprintf(x, y, "Press any key to exit help");
	free(x);
	free(y);
}
...
void DrawStimWindow(void)
{
	...
	char *a, buf[80];
	char b[8];
	char *stimprotocol[] = {"Time", "Area", "Programmed", "Random"};
	char *stimmode[] = {"None", "Test only", "Control only", 
	   "Both Test & Control", "Programmed"};
    ...
    drwbox(SET, _RED, sysinfo.helpview.x, sysinfo.helpview.y, 
 	...
 	/* draw the title */
    setcolor(WHITE);
    sprintf(tmpstring, "AD Stim Params Window");
    /* x and y determine the coordinates of the beginning positions of the
       help strings. The tmpstring title will be centered in the window */ 
    *x = (sysinfo.helpview.x + sysinfo.helpview.x2 - textwidth(tmpstring)) / 2; 
    *y = sysinfo.helpview.y + 2 * sysinfo.theight;
    ...
    *x = sysinfo.helpview.x + sysinfo.twidth / 2; 
    *y = *y + 2 * sysinfo.theight;
	gprintf(x, y, " %        Percent active stimulation");
	gprintf(x, y, " v        Stim pulse magnitude, Volts");
	gprintf(x, y, " t        Set test pattern electrodes");
	gprintf(x, y, " c        Set control pattern electrodes");
	gprintf(x, y, " a        Area of stimulation, from tracker");
	gprintf(x, y, " l        Pulse length, msec.");
	gprintf(x, y, " n        Number of pulses in burst");
	gprintf(x, y, " i        Interpulse interval within a burst");
	gprintf(x, y, " d        Delay between bursts");
	gprintf(x, y, " ->       Increase Phase w.r.t. theta");
	gprintf(x, y, " <-       Decrease Phase w.r.t. theta");
	gprintf(x, y, " p        Set stimulation protocol Mode");
	gprintf(x, y, " s        Save stimulation parameters to file.");
	gprintf(x, y, " r        Read stimulation parameters from file.");
	gprintf(x, y, " o        Toggle stimulation On/Off.");
	gprintf(x, y, " g        Grid lines On/Off (Tracker mode).");
	*y = *y + 30;
	gprintf(x, y,"           CURRENT STIM PARAMETERS:           ");
	gprintf(x, y," ");
	gprintf(x, y,"Stimulation on %d%% of trials.",sysinfo.stim.percent);
	gprintf(x, y,"Stim Voltage:        %.2f Volts.",sysinfo.stim.voltage);
	gprintf(x, y,"Stim. pulse length:   %.2f msec.",(float)sysinfo.stim.pulselen/100);
	gprintf(x, y,"# of pulses in burst: %d",sysinfo.stim.number);
	gprintf(x, y,"Interpulse interval:  %.2f msec.",(float)sysinfo.stim.interpulse/100);
	if (sysinfo.stim.protocol == PROGRAMMED)
	   gprintf(x, y,"Delay between bursts: Programmed.");
	else gprintf(x, y,"Delay between bursts: %.2f sec.",(float)sysinfo.stim.delay/10000);
	gprintf(x, y,"Phase w.r.t Theta:    %d degrees.",sysinfo.stim.phase);
    gprintf(x, y,"Stimulation protocol: %s.", stimprotocol[sysinfo.stim.protocol]);
	gprintf(x, y,"Stimulation mode: %s.", stimmode[sysinfo.stim.mode]);
	gprintf(x, y," ");
//	strcpy(buf,"Test pattern electrodes:     ");
	sprintf(buf,"Test pattern electrodes:     ");
	if (sysinfo.stim.test[0] == 0) gprintf(x, y,
	    "Test pattern electrodes:     None assigned.");
	else
	 {
	  for (i = 0; i < sysinfo.stim.test[0]; i ++) 
	   {
		sprintf(a,"%d ",sysinfo.stim.test[i+1]);
		strcat(buf,a);
		}
	  gprintf(x, y,"%s",buf);
	 }
	 sprintf(buf,"Control pattern electrodes:  ");
	 if (sysinfo.stim.control[0] == 0) gprintf(x, y,
	    "Control pattern electrodes:   None assigned.");
	 else
	 {
	  for (i = 0; i < sysinfo.stim.control[0]; i ++) 
	   {
		sprintf(a,"%d ",sysinfo.stim.control[i+1]);
		strcat(buf,a);
		}
	  gprintf(x, y,"%s",buf);
	 }
    gprintf(x, y," ");
	 if (sysinfo.stim.tstarea.x2 == 0) gprintf(x, y,
	 	"Area of test stimulation: none.");
	 else {
	  gprintf(x, y,"Area of test stimulation (green box):");
	  gprintf(x, y,"x-bounds: %d - %d%%",
	   sysinfo.stim.tstarea.x, sysinfo.stim.tstarea.x2);
	 if (sysinfo.stim.tstarea.y2 == 0) gprintf(x, y,"y-bounds: none");
	 else gprintf(x, y,"y-bounds: %d - %d%%",
	   sysinfo.stim.tstarea.y, sysinfo.stim.tstarea.y2);
		}
	 if (sysinfo.stim.ctlarea.x2 == 0) gprintf(x, y,
	 	"Area of control stimulation: none.");
	 else {
	 gprintf(x, y,"Area of control stimulation (yellow box):");
	 gprintf(x, y,"x-bounds: %d - %d%%",
	   sysinfo.stim.ctlarea.x, sysinfo.stim.ctlarea.x2);
	 if (sysinfo.stim.ctlarea.y2 == 0) gprintf(x, y,"y-bounds: none");
	 else gprintf(x, y,"y-bounds: %d - %d%%",
	   sysinfo.stim.ctlarea.y, sysinfo.stim.ctlarea.y2);
	 }
	if (sysinfo.mode == TRACKER_MODE)
	 {
	 ClearTrackerBox();
	 DrawTrackerBorder();
	 p = (float)(sysinfo.trackview.x2 - sysinfo.trackview.x - 2)/100; 
	 q = (float)(sysinfo.trackview.y2 - sysinfo.trackview.y - 2)/100; 
	 i = sysinfo.trackview.x + 1 + p*sysinfo.stim.tstarea.x;
	 j = sysinfo.trackview.y + 1 + q*sysinfo.stim.tstarea.y;
	 k = sysinfo.trackview.x + 1 + p*sysinfo.stim.tstarea.x2; 
	 l = sysinfo.trackview.y + 1 + q*sysinfo.stim.tstarea.y2;
	 if ((i < k) && (j < l)) drwbox(SET, _GREEN, i, j, k, l);
	 i = sysinfo.trackview.x + 1 + p*sysinfo.stim.ctlarea.x;
	 j = sysinfo.trackview.y + 1 + q*sysinfo.stim.ctlarea.y;
	 k = sysinfo.trackview.x + 1 + p*sysinfo.stim.ctlarea.x2; 
	 l = sysinfo.trackview.y + 1 + q*sysinfo.stim.ctlarea.y2;
	 if ((i < k) && (j < l)) drwbox(SET, _YELLOW, i, j, k, l);
	 setcolor(WHITE);
	 }
	gprintf(x, y," ");
	if (sysinfo.stim.protocol == PROGRAMMED) {
	   ...
	   for (i = 0;  ++i <= sysinfo.stim.seqlength;) {
		sprintf(b, " %.1f",sysinfo.stim.seq[i]/10000.);
		strcat(a, b); }
		gprintf(x, y, "%s", a);
	}
	gprintf(x, y," ");
	...
	else sprintf(tmpstring2, "Stimulation is off");
	ErrorMessage(tmpstring2);
	/* replace this with sequence list */
	if (sysinfo.seq[STIMSEQON].active) gprintf(x, y,"Stimulation is ON ");
	else gprintf(x, y,"Stimulation is OFF ");
	*y = sysinfo.helpview.y2 - 2 * sysinfo.theight;
	gprintf(x, y, "Press return to exit");
	free(a);
	free(x);
	free(y);
}
...
void DrawTrackerGrid(void)
{
	...
	float p, q;
	 ...
	 p = (float)(sysinfo.trackview.x2 - sysinfo.trackview.x - 2)/100; 
	 q = (float)(sysinfo.trackview.y2 - sysinfo.trackview.y - 2)/100; 
	 if (sysinfo.stim.grid) {
	   for (i = 0;  ++i < 10;) {
	      drwline(SET, _BLUE, sysinfo.trackview.x + 10*i*p, sysinfo.trackview.y,
		         sysinfo.trackview.x + 10*i*p, sysinfo.trackview.y2);
	      *x = sysinfo.trackview.x + 10*i*p;
		  *y = sysinfo.trackview.y - 10; 
		  gprintf(x, y, "%d", 10*i);
	      *y = sysinfo.trackview.y2 + 5; 
		  gprintf(x, y, "%d", 10*i);
	      drwline(SET, _BLUE, sysinfo.trackview.x, sysinfo.trackview.y + 10*i*q,
		         sysinfo.trackview.x2, sysinfo.trackview.y + 10*i*q);
	      *x = sysinfo.trackview.x - 15;
		  *y = sysinfo.trackview.y + 10*i*q-5;
		  gprintf(x, y, "%d", 10*i);
		  }
	   }
	 else {
...
void DrawStimProtocol(void)
{
    ...
    drwbox(SET, _RED, sysinfo.helpview.x, sysinfo.helpview.y, 
 	...
 	/* draw the title */
    setcolor(WHITE);
    sprintf(tmpstring, "AD Stim Protocol");
    /* x and y determine the coordinates of the beginning positions of the
       help strings. The tmpstring title will be centered in the window */ 
    *x = (sysinfo.helpview.x + sysinfo.helpview.x2 - textwidth(tmpstring)) / 2; 
    *y = sysinfo.helpview.y + 2 * sysinfo.theight;
    ...
    *x = sysinfo.helpview.x + sysinfo.twidth / 2; 
    *y = *y + 2 * sysinfo.theight;
	gprintf(x, y, "Tetrode Configuration:");
	gprintf(x, y, " n        No pulses output");
	gprintf(x, y, " t        Test pulses only");
	gprintf(x, y, " c        Control pulses only");
	gprintf(x, y, " b        Both test and control pulses");
	gprintf(x, y, " ");
	gprintf(x, y, "Stimulation Protocol:");
	gprintf(x, y, " i        Time mode - single interval");
	gprintf(x, y, " a        Area mode: trigger on position");
	gprintf(x, y, " p        Programmable");
	gprintf(x, y, " r        Random selection, test or control");
	gprintf(x, y, " ");
	gprintf(x, y, "Press any other key to exit");
	gprintf(x, y, " ");
	gprintf(x, y, " ");
	if (sysinfo.stim.protocol == PROGRAMMED) {
	   gprintf(x, y, "Programmed delay sequence: ");
	   for (i = 0;  ++i <= sysinfo.stim.seqlength;) {
		z = sysinfo.stim.seq[i]/10000.;
		gprintf(x, y, "       %.3f sec.",z);   }
	}
	free(x);
	free(y);
}
...
int tmpval;
						...
						if (sysinfo.mode == SPIKE_MODE) {
							...
							for (tmpval = 0; tmpval < adinfo.nelectrodes; 
					 			tmpval++) 
...
void DrawSpikeBorder(int e_num, int channel)
{
    ...
    drwline(SET, _RED, spikewin->x, electrode->yoffset, spikewin->x+3, 
	 ...
	 /* draw the display threshold line */		
	 x = electrode->spikexoffset[channel] - 1 + 
	              (int) ((float) electrode->displaythresh / 
	              (float) SINGLESPIKELEN * (spikewin->x2 - spikewin->x));
					
    drwline(SET, LIGHTGRAY, x, spikewin->y, x, spikewin->y + 5); 
}
...
/*   if(sysinfo.debug){
   ...
   if((supcsr & BIT9) == BIT9){
	...
	if(adinfo.nbuffers > 0){
   sprintf(tmpstring, "Er:%ld C/T:%lf Pk:%d Dk:%ld Dp:%ld Sp:%ld Me:%ld",
           adinfo.error_count, (double) adinfo.computed_used / 
		   (double) adinfo.nbuffers,
		   adinfo.peak_rate, adinfo.disk_lost, adinfo.display_lost,
           adinfo.nspikes,farcoreleft());
   } else {
   sprintf(tmpstring, "Er:%ld Dk:%ld Dp:%ld Me:%ld",
           adinfo.error_count, 
		   adinfo.disk_lost, adinfo.display_lost,
           farcoreleft());
   }
...
void DrawContinuousTicks(void)
{
	...
	*/
	...
	/*
	** label the trace with the channel number
	*/
	sprintf(str,"%d",i+1);
	outtextxy(0, cptr->contoffset, str);   
        /* 
	...
	*/
	...
	/*
	** draw arrows indicating threshold levels
	*/
        if (cptr->contthreshtick > sysinfo.scopeview.y + 1){
	    DrawArrow(cptr->color,sysinfo.scopeview.x,cptr->contthreshtick);	
	}
	/*
	sprintf(str,"%d",cptr->thresh);
	outtextxy(0, cptr->contthreshtick, str);   
	*/
        cptr++;
   }
}
...
int	startoffset;
   ...
   overlay = sysinfo.contoverlay;
    ...
    /*
    ** compute the xaxis scale factor such that a single buffer fills
    ** the entire screen.
    */
    /*
    ** make sure that the desired timebase is valid
    */
    if(sysinfo.desired_timebase > 0){
	iscale = (sysinfo.scopeview.x2 - sysinfo.scopeview.x - 5) /
	    ((double)sysinfo.desired_timebase*adinfo.rate_set/adinfo.nchannels);
    } else {
	/*
	** default to displaying one buffer per screen
	*/
	iscale = (sysinfo.scopeview.x2 - sysinfo.scopeview.x - 5) /
            ((double)adinfo.dma_bufsize/adinfo.nchannels);
    }
    /*
    ** if triggering is set then start the display 
    ** at the trigger point rather than at the start of the buffer.
    ** But dont trigger until the end of a sweep (ie until display
    ** reaches the end of the screen
    ** Note that a threshold crossing on any channel will trigger
    ** a sweep display on all channels
    **
    ** note that a triggered event within a buffer will prevent
    */
    if(sysinfo.triggered_continuous && (sysinfo.trigger == 0) && sysinfo.xoffset==0){
	return;
	
    }

    if((sysinfo.trigger > 0) && (sysinfo.xoffset == 0)){
	    /*
	    ** the trigger contains the buffer offset of channel zero at the 
	    ** trigger point
	    */
	    startoffset = sysinfo.trigger;
	    dataptr += startoffset;
    } else {
	startoffset = 0;
    }
   ...
   offset = ADOFFSET;
   /*
   ** we can maintain a display of more than one buffer if we dont
   ** reset the xoffset until reaching the end of the screen
   ** In this case the iscale would be set to a multiple of the buffer size
   */
   if((sysinfo.xoffset >= sysinfo.scopeview.x2) || sysinfo.xoffset <= 0){
       sysinfo.xoffset = (double) sysinfo.scopeview.x + 5;
       sysinfo.continuous_pixelindex = 0;
   }
   lastxoffset = (int) sysinfo.xoffset;
   prevptr = adinfo.prevptr + sysinfo.continuous_pixelindex*adinfo.nchannels;
   for(i = startoffset; i < bufsize; i+= adinfo.nchannels){
      cptr = adinfo.channel;
      sysinfo.xoffset += iscale;
      /*
      ** dont display past the edge of the screen
      */
      if(sysinfo.xoffset > sysinfo.scopeview.x2){
	sysinfo.xoffset = 0;
	break;
	}
      /* only draw the point if it will not overlap with the last point */
      if ((int) sysinfo.xoffset > lastxoffset) {
         lastxoffset = (int) sysinfo.xoffset;
	 ...
	 sysinfo.continuous_pixelindex++;
      ...
      if(adinfo.dmadone || adinfo.error){
	 sysinfo.xoffset = 0;
	 break;
      }
   }
    ...
    */
    for (i = 0; i < electrode->nspikes; i++) {
        ...
        for(j = 0; j < adinfo.nelect_chan; j++) {
 	    ...
 	    if (electrode->displaythresh >= SINGLESPIKELEN - 1) {
			setmaxzero = 0;
		}		
		  else {		
				for(j = 0; j < adinfo.nelect_chan; j++) {
					 /* if the displaythresh value is above threshold, set 
					     the max to 0*/
						  if (*(spikedataptr + electrode->displaythresh + 1) < 
								electrode->thresh[j]) {
								setmaxzero = 0;
								break;
						  }
